    ...
    <div className="flex justify-between items-end px-2 py-4 mt-20 mb-8 border-b border-white/5 select-none">
      ...
      <div className="flex items-center gap-6">
        <div className="flex flex-col items-end">
          <span className="text-[8px] tracking-[0.4em] font-black text-white/10 uppercase mb-2">Haze Vector</span>
          <div className="flex gap-1">
            {[1, 2, 3, 4, 5].map((s) => (
              <div 
                key={s} 
                className={`w-4 h-1 rounded-full transition-all duration-700 ${
                  sipLevel > (s * 20) ? 'bg-amber-500/60 shadow-[0_0_10px_rgba(245,158,11,0.3)]' : 'bg-white/5'
                }`} 
              />
            ))}
          </div>
